...
function Historial() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] pb-32 font-sans selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{filteredData.length} registros</p>
            </div>
          </div>
      ...
      <main className="max-w-2xl mx-auto px-6 mt-8 space-y-3">
        {loading ? (
          ...
          <div className="py-20 text-center">
            <LayoutGrid size={48} className="mx-auto text-slate-100 mb-4" strokeWidth={1} />
            <p className="text-slate-400 text-sm font-bold">Sin resultados</p>
          </div>
        ) : (
          <>
            {displayedData.map((item) => (
              ...
              >
                <div className="relative shrink-0">
                  {item.evidenciaFoto ? (
                    <img src={item.evidenciaFoto} className="w-14 h-14 rounded-2xl object-cover" alt="" loading="lazy" decoding="async" />
                  ) : (
            ...
            {hasMore && (
              <button 
                ...
                className="w-full py-4 mt-4 bg-white border border-slate-100 rounded-2xl flex items-center justify-center gap-2 text-slate-500 hover:bg-slate-50 hover:text-indigo-600 transition-all group shadow-sm"
              >
                <span className="text-xs font-black uppercase tracking-widest">Ver más registros</span>
      ...
      {modalOpen && selectedItem && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-[32px] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-full duration-500">
            ...
            <div className="px-8 pt-6 pb-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-base font-black text-slate-900 tracking-tight">Detalles</h2>
              </div>
            ...
            <div className="px-8 pb-8">
              ...
              <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar">
                {[selectedItem.evidenciaFoto, selectedItem.evidenciaDanio].filter(Boolean).map((img, idx) => (
                  <div key={idx} className="relative shrink-0 group cursor-pointer" onClick={() => setImagePreview(img)}>
                    <img src={img} className="w-48 h-32 object-cover rounded-2xl border border-slate-100 shadow-sm" alt="" />
                    <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl flex items-center justify-center text-white">
                      <Eye size={20} />
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3 mt-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <InfoBox label="Cantidad" value={selectedItem.cantidadEntregada} />
                </div>
                <InfoRow icon={<User size={16}/>} label="Entregado por" value={selectedItem.entregadoPor} />
                <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100/50">
                  <div className="flex items-center gap-2 text-slate-400">
                    <Calendar size={14} />
                    <span className="text-[11px] font-bold uppercase tracking-tighter">Fecha</span>
                  </div>
                  <span className="text-xs font-black text-slate-700">{formatFecha(selectedItem.fecha)}</span>
                </div>
              </div>

              <button onClick={() => setModalOpen(false)} className="w-full mt-8 bg-slate-900 py-4 rounded-2xl text-white font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all active:scale-[0.98]">
                Cerrar
              </button>
            </div>
          </div>
      ...
      {/* ── PREVIEW IMAGEN ── */}
      {imagePreview && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={() => setImagePreview(null)}>
          <div className="absolute inset-0 bg-black/90 backdrop-blur-sm" />
          <img src={imagePreview} className="relative max-w-full max-h-[80vh] rounded-3xl shadow-2xl" alt="" />
...
const InfoRow = ({ icon, label, value }) => (
  <div className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100/50">
    ...
    <span className="text-sm font-extrabold text-slate-800 truncate max-w-[150px]">{value || 'N/A'}</span>
  </div>
);
